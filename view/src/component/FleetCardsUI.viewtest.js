...
const criticalInstancesByShip = state => shipId =>
  AS.Selector.criticalInstancesByShip(shipId, state);
...
const createFleetCardsUI = (fleetId, state) => {
  ...
  const reduceFunction0 = (accum, shipId) =>
    R.assoc(shipId, upgradeInstancesByShip0(shipId), accum);
  ...
  const reduceFunction1 = (accum, shipId) =>
    R.assoc(shipId, criticalInstancesByShip0(shipId), accum);
  const shipToDamages = R.reduce(reduceFunction1, {}, shipIds);
  const reduceFunction2 = (accum, shipInstance) =>
    R.assoc(
      shipInstance.id,
      AS.Selector.defenseTokenInstancesByShip(shipInstance.id, state),
      accum,
    );
