...
const createCommandUI = (command, isSmall, showLabel) =>
...
const mapFunction0 = command => prop =>
  ReactUtilities.createCell(createCommandUI(command, prop[0], prop[1]), prop[2], className);
const createCells = command => R.map(mapFunction0(command), props);

const commands = AA.EnumUtilities.values(AA.Command);
let i = 0;
const mapFunction1 = command => {
  i += 1;
  return ReactUtilities.createRow(createCells(command), i);
};
