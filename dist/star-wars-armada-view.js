(function (global, factory) {
  ...
  const createFooterRow = (ships, shipToUpgrades, squadrons) => {
    ...
    const cells = [
      ...
      ReactUtilities.createCell(attack1 + attack3, "attackCell", className),
      ReactUtilities.createCell(hull1 + hull3, "hullCell", className),
      ReactUtilities.createCell(shield1, "shieldCell", className),
      ReactUtilities.createCell(points1 + points2 + points3, "pointsCell", className)
    ];
  ...
  const createHeaderRow = () => {
    ...
    const cells = [
      ReactUtilities.createCell("Ship / Squadron", "nameCell", className),
  ...
  const createShipRow = ship => {
    ...
    const cells = [
      ...
      ReactUtilities.createCell(computeShipAttack(shipCard), "attackCell", NUMBER_CLASS),
      ReactUtilities.createCell(shipCard.hull, "hullCell", NUMBER_CLASS),
      ReactUtilities.createCell(computeShipShield(shipCard), "shieldCell", NUMBER_CLASS),
      ReactUtilities.createCell(shipCard.points, "pointsCell", NUMBER_CLASS)
    ];
  ...
  const createSquadronRow = squadron => {
    ...
    const cells = [
      ReactUtilities.createCell(imageWithLabelUI, "nameCell", TEXT_CLASS),
      ReactUtilities.createCell(computeSquadronAttack(squadronCard), "attackCell", NUMBER_CLASS),
      ReactUtilities.createCell(squadronCard.hull, "hullCell", NUMBER_CLASS),
      ReactUtilities.createCell("", "shieldCell", NUMBER_CLASS),
      ReactUtilities.createCell(squadronCard.points, "pointsCell", NUMBER_CLASS)
    ];
    const key = `squadronRow${squadron.id}`;

    return ReactUtilities.createRow(cells, key, "bg-xw-medium", { title: squadronCard.text });
  };
  ...
  const createUpgradeRows = (upgradeInstances, key) =>
    R.map(upgrade => {
      ...
      const cells = [
        ReactUtilities.createCell(imageWithLabelUI, "nameCell", TEXT_CLASS),
        ReactUtilities.createCell("", "attackCell", NUMBER_CLASS),
        ReactUtilities.createCell("", "hullCell", NUMBER_CLASS),
        ReactUtilities.createCell("", "shieldCell", NUMBER_CLASS),
  ...
  class FleetTable extends React.PureComponent {
    render() {
      ...
      return ReactUtilities.createTable(rows, "fleetTable", "ba b--black bg-xw-light collapse fl f6");
    }
  }
  ...
  exports.FleetTableContainer = FleetTableContainer;
